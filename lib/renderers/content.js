    ...
    weld(dom, data, {
      map: function(parent, element, key, val) {
        ...
        if ($(element).hasClass("title")) {
          var articlePath = helpers.unresolve(smith.src, metadata.link);

          if (metadata.render !== "directory") {
            // The leading dot in the link is not valid for some web server.
...
content.generate = function(output, pages) {
  ...
  Object.keys(pages).forEach(function (file) {
    ...
    if (content) {
      ...
      // TODO: Get the doctype of the file from the content object.
      fs2.writeFile(newPath, "<!DOCTYPE html>" + pages[file].content, function (err) {
        if (err) {
